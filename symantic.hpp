...
public:
    ...
    Scope(std::string Val,
          vector<string> Type,
          int Offset, bool is_func = false, bool is_ovevride = false)
    {
        this->Val = Val;
        this->Offset = Offset;
        this->Type = Type;
...
public:
    ...
    Exp(string op, Exp *A, Exp *Beta, Node* op_sign, Prev* prev);
    Exp(Node *id);
    Exp(string Mtype, Node *N);	//NUM
    Exp(string Mtype, string bool_val);
//    Exp(string Mtype, string bool_val) : Node(bool_val), _type(Mtype) {
...
//        true_list = T;
//            if (bool_val== "true"){
//                buffer_liad.emit(this->_reg + " = add i1 0,1");
//            }
//            else if (bool_val== "false"){
//                buffer_liad.emit( this->_reg + " = add i1 0,0");
//            }
//
//    }
    Exp(Type *type, Exp *E);
